...
# STARTING FA FEATURE EXTRACTION
sc = StandardScaler()
...
bgclassifier = BaggingClassifier(estimator=LogisticRegression(random_state=1), n_estimators=500,
                                 max_features=11,
                                 max_samples=12,
                                 random_state=1, n_jobs=5)
bgclassifier.fit(X_FA, y)

y_pred = cross_val_predict(bgclassifier, X_FA, y, cv=5)
print("************************ FA FEATURE EXTRACTION ************************")
...
res = cross_val_score(bgclassifier, X_FA, y, cv=5, scoring='accuracy')
print("Average Accuracy: \t {0:.4f}".format(np.mean(res)))
print("Accuracy SD: \t\t {0:.4f}".format(np.std(res)))
fpr, tpr, thresholds = metrics.roc_curve(y, y_pred)
roc_auc = metrics.auc(fpr, tpr)
display_roc = metrics.RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=roc_auc,estimator_name='Logistic Regression With FA Feature Extraction')
display_roc.plot()
plt.show()
